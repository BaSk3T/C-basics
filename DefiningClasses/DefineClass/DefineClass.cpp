...
//Define several constructors for the defined classes that take different sets of arguments(the full information for the class
//or part of it).
...
int main()
{
	Battery *battery = new Battery("model", BatteryType::NiMH, 502, 30);
	Display *display = new Display(4.5, 100000);

	GSM *phone = new GSM("Model", "Manufactrurer", 14, "Me", battery, display);

	GSMTest gsmTester;

	gsmTester.printGSMs();
	gsmTester.printIPhone();

	std::cout << "----------------------------------------------" << std::endl;

	GSMCallHistoryTest *gsmCallHistoryTest = new GSMCallHistoryTest(phone, 37);
	gsmCallHistoryTest->displayCalls();
	gsmCallHistoryTest->displayTotalPriceOfCalls();
	gsmCallHistoryTest->displayPriceWithoutLongestCall();
	gsmCallHistoryTest->displayClearedCallHisory();
